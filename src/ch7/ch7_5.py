...
def train(model, traindataloader, train_rate, criterion, optim, device, lr = 0.01, num_epochs = 25,
    ...
    for epoch in range(num_epochs):
        ...
        tb.add_row(
            [str(epoch + 1), str(round(train_loss_all[-1], 4)), str(round(val_loss_all[-1], 4)),
             str(round(train_acc_all[-1], 4)), str(round(val_acc_all[-1], 4)), str(round(test_acc, 4))])
        print(tb)
...
history = train(net, train_iter, 0.8, loss, optimizer_sgd, d2l.try_gpu(), lr = lr, kernel_regularizer = 0,
                num_epochs = num_epochs, saved_path = 'batchnorm_alexnet.pt')
...
##
net.load_state_dict(
    torch.load(path.weight_path + "batchnorm_alexnet.pt",
