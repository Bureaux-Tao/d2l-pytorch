...
class Inception(nn.Module):
    ...
    # def forward(self, x):
    #     p1 = self.p1_1(x)
    #     p1 = nn.functional.relu(p1)
    #
    #     p2 = self.p2_1(x)
    #     p2 = nn.functional.relu(p2)
    #     p2 = self.p2_2(p2)
    #     p2 = nn.functional.relu(p2)
    #
    #     p3 = self.p3_1(x)
    #     p3 = nn.functional.relu(p3)
    #     p3 = self.p3_2(p3)
    #     p3 = nn.functional.relu(p3)
    #
    #     p4 = self.p4_1(x)
    #     p4 = nn.functional.relu(p4)
    #     p4 = self.p4_2(p4)
    #     p4 = nn.functional.relu(p4)
    #     # 在通道维度上连结输出
    ...
    def forward(self, x):
        ...
        # 在通道维度上连结输出
        return torch.cat((route_1(x), route_2(x), route_3(x), route_4(x)), dim = 1)
...
def train(model, traindataloader, train_rate, criterion, optimizer, device, num_epochs = 25,
    ...
    val_loss_all = []
    val_acc_all = []
    reduceLR = ReduceLROnPlateau(optimizer, factor = 0.25, patience = 3, min_lr = 1e-8, verbose = True)
...
optimizer_sgd = torch.optim.SGD(net.parameters(), lr = lr, weight_decay = 0.001)

##
history_sgd = train(net, train_iter, 0.8, loss, optimizer_sgd, d2l.try_gpu(), 100, 'Inception.pt')
...
##
net.load_state_dict(
    torch.load(path.weight_path + "Inception.pt",
